...
Shortly after I had the basic prototype of my game I received a lot of feedback from players that related to `lag'. The `lag' they were referring to was the latency between their interaction and visual/audible feedback. This detracts from the responsiveness and tactile impression it should give the player. After some research I found an article written by \cite{Croft} on creating responsive buttons with PhoneGap (Cordova), it encouraged me to test different events and I found that \texttt{touchstart} and \texttt{touchend} appeared to function much better than \texttt{mousedown} and \texttt{mouseup} events which I was previously using. The only issue is that there is no touch event equivalent for \texttt{mouseout} used to detect when the user as moved off the element when pressing. I found a polyfill that suited 
